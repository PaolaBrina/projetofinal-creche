...
export default function CalendarioResponsavel({ navigation, route }) {
...
export default function CalendarioResponsavel({ navigation, route }) {
    ...
    const [loading, setLoading] = useState(true);
    const [noMaterials, setNoMaterials] = useState(false);
    const [openedItem, setOpenedItem] = useState(null); // Estado para armazenar o índice do item aberto
    ...
    // Função para alternar a exibição do item
    const toggleItem = (index) => {
        // Se o item já estiver aberto, fechamos ele (ou abrimos o novo)
        setOpenedItem(openedItem === index ? null : index);
    };

    // Verifica se não há materiais e exibe mensagem apropriada
    const renderEmptyState = () => (
        <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>Calendário não cadastrado para a escola ainda.</Text>
        </View>
    ...
    // Renderiza os itens da lista (calendários)
    const renderItem = ({ item, index }) => (
        <View style={styles.alunoItemContainer}>
            <TouchableOpacity onPress={() => toggleItem(index)}>
                <View style={styles.alunoRow}>
